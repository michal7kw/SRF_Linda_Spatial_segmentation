...
output_dir = 'MERGE_SPATIAL_DATA/marker_genes_per_sample_inclusive_new_clusters'
...
for sample_id in samples:
    try:
        ...
        # Update leiden_new categories to only include present clusters
        sample_adata.obs['leiden_new'] = sample_adata.obs['leiden_new'].cat.remove_unused_categories()
        
        # Identify clusters with more than one cell for differential expression analysis
        cluster_counts = sample_adata.obs['leiden_new'].value_counts()
        valid_clusters_for_de = cluster_counts[cluster_counts > 1].index.tolist()
        
        print(f"Analyzing {len(sample_adata.obs['leiden_new'].cat.categories)} clusters for sample {sample_id}")
        ...
        n_top_genes = 30 # Define here for scope

        if len(valid_clusters_for_de) < 2:
            print(f"Skipping differential expression analysis and related plots for {sample_id}: insufficient clusters with >1 cell for comparison.")
            # Create a dummy entry in uns to prevent errors in overall summary if it expects this key
            sample_adata.uns['rank_genes_leiden_new'] = {'names': np.array([]), 'scores': np.array([]), 'pvals': np.array([]), 'pvals_adj': np.array([]), 'logfoldchanges': np.array([])}
            # Skip the rest of the processing for this sample
        else:
            # Perform differential expression analysis for this sample on valid clusters
            print(f"Performing differential expression analysis for {sample_id} on valid clusters...")
            
            sc.tl.rank_genes_groups(
                sample_adata, 
                'leiden_new', 
                groups=valid_clusters_for_de, # Only analyze clusters with >1 cell
                method='wilcoxon',
                key_added='rank_genes_leiden_new',
                n_genes=50
            )
            ...
            marker_genes_list = []
            # Iterate only over the clusters that were actually analyzed
            for group in valid_clusters_for_de: 
                group_df = sc.get.rank_genes_groups_df(sample_adata, group=group, key='rank_genes_leiden_new')
                group_df['group'] = group
                group_df['sample_id'] = sample_id
                marker_genes_list.append(group_df)
            
            if marker_genes_list: # Only concatenate if there are results
            ...
            if top_markers_per_cluster: # Only create DataFrame if there are top markers
            ...
            for cluster in sorted(valid_clusters_for_de, key=int): # Iterate only over valid clusters
                cluster_cells = sample_adata[sample_adata.obs['leiden_new'] == cluster]
...
- Total clusters found: {len(cluster_counts)}
...
overall_stats = []
for sample_id in samples:
    sample_adata = adata[adata.obs['sample_id'] == sample_id].copy() # Use .copy() to avoid SettingWithCopyWarning
    sample_adata.obs['leiden_new'] = sample_adata.obs['leiden_new'].cat.remove_unused_categories()
    cluster_counts = sample_adata.obs['leiden_new'].value_counts()
    
    # Recalculate valid_clusters_for_de for the summary, as it might have been skipped in the loop
    valid_clusters_for_de_summary = cluster_counts[cluster_counts > 1].index.tolist()

    overall_stats.append({
        'sample_id': sample_id,
        'total_cells': sample_adata.n_obs,
        'total_clusters_found': len(cluster_counts),
...
Per-Sample Results:
{chr(10).join([f"- {row['sample_id']}: {row['total_cells']} cells, {row['total_clusters_found']} clusters found, {row['clusters_analyzed_for_de']} clusters analyzed for DE" for _, row in overall_stats_df.iterrows()])}
