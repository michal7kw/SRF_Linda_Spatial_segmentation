...
output_dir = 'MERGE_SPATIAL_DATA/plots_hippo_cluster_highlight_inclusive_new_clusters'
...
for sample_id in merged_adata.obs['sample_id'].unique():
    ...
    cluster_counts = sample_data.obs['leiden_new'].value_counts().sort_index()
...
# %%
# Plot leiden_new clusters spatially, faceted by sample
sq.pl.spatial_scatter(merged_adata, color='leiden_new', shape=None,
                      library_key='sample_id',
                      save='spatial_leiden_new_by_sample.png')

# %%
# Plot leiden_new clusters spatially for each sample
for sample_id in merged_adata.obs['sample_id'].unique():
    sample_adata = merged_adata[merged_adata.obs['sample_id'] == sample_id].copy()
    
    title_text = f"Spatial leiden_new Clusters - Sample: {sample_id}"
    
    print(f"Plotting spatial scatter for sample: {sample_id}")
    sq.pl.spatial_scatter(sample_adata, color='leiden_new', shape=None,
                          title=title_text,
                          save=f'spatial_leiden_new_by_sample_{sample_id}.png')

# %%
# Get unique samples and clusters
samples = merged_adata.obs['sample_id'].unique()
clusters = merged_adata.obs['leiden_new'].unique()
...
for sample in samples:
    ...
    for cluster in sorted(sample_adata.obs['leiden_new'].unique()):
        cluster_mask = sample_adata.obs['leiden_new'] == cluster
        ...
        # Get the color for this cluster from leiden_new colors
        if 'leiden_new_colors' in merged_adata.uns:
            cluster_colors = dict(zip(merged_adata.obs['leiden_new'].cat.categories, merged_adata.uns['leiden_new_colors']))
        ...
        full_cluster_mask = (merged_adata.obs['leiden_new'] == cluster) & sample_mask
