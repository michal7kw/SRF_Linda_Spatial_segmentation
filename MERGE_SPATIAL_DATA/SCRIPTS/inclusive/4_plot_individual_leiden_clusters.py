...
# %%
# Get leiden clusters
unique_clusters = sorted(merged_adata.obs['leiden'].unique())
print(f"Found {len(unique_clusters)} leiden clusters: {unique_clusters}")

# %%
# Create output directory
output_dir = 'MERGE_SPATIAL_DATA/plots_individual_leiden_clusters'
...
for i, cluster in enumerate(unique_clusters):
    ...
    merged_adata.obs.loc[merged_adata.obs['leiden'] == cluster, highlight_col] = f'Cluster {cluster}'
    ...
    # Customize the plot
    ax.set_title(f'Leiden Cluster {cluster}', fontsize=12, fontweight='bold')
...
# Add main title
fig.suptitle('Individual Leiden Clusters - UMAP Visualization', fontsize=16, fontweight='bold', y=0.98)

# Save the figure
output_path = os.path.join(output_dir, 'individual_leiden_clusters_grid.png')
...
for cluster in unique_clusters:
    ...
    sc.pl.umap(merged_adata, 
               ...
               show=False,
               frameon=True,
               title=f'Leiden Cluster {cluster}',
               legend_loc='right margin')
    
    # Customize
    ax.set_title(f'Leiden Cluster {cluster}', fontsize=14, fontweight='bold')
    ax.set_xlabel('UMAP 1', fontsize=12)
    ax.set_ylabel('UMAP 2', fontsize=12)
    
    # Save individual plot
    individual_path = os.path.join(output_dir, f'leiden_cluster_{cluster}_individual.png')
...
sc.pl.umap(merged_adata, 
           ...
           show=False,
           frameon=True,
           title='All Leiden Clusters',
           legend_loc='right margin')

ax.set_title('All Leiden Clusters - Overview', fontsize=14, fontweight='bold')
ax.set_xlabel('UMAP 1', fontsize=12)
ax.set_ylabel('UMAP 2', fontsize=12)

# Save overview plot
overview_path = os.path.join(output_dir, 'all_leiden_clusters_overview.png')
...
print(f"  - Grid plot: individual_leiden_clusters_grid.png")
print(f"  - Overview plot: all_leiden_clusters_overview.png")
for cluster in unique_clusters:
    print(f"  - Individual plot: leiden_cluster_{cluster}_individual.png")
