...
output_dir = 'MERGE_SPATIAL_DATA/marker_genes_per_sample_inclusive'
...
for sample_id in samples:
    try:
        ...
        sample_adata.obs['leiden'] = sample_adata.obs['leiden'].cat.remove_unused_categories()
        ...
        print(f"Analyzing {len(sample_adata.obs['leiden'].cat.categories)} clusters for sample {sample_id}")
        ...
        if len(valid_clusters_for_de) < 2:
            ...
            sample_adata.uns['rank_genes_leiden'] = {'names': np.array([]), 'scores': np.array([]), 'pvals': np.array([]), 'pvals_adj': np.array([]), 'logfoldchanges': np.array([])}
            ...
            for group in valid_clusters_for_de: 
                group_df = sc.get.rank_genes_groups_df(sample_adata, group=group, key='rank_genes_leiden')
            ...
            for cluster in sorted(valid_clusters_for_de, key=int): # Iterate only over valid clusters
                cluster_cells = sample_adata[sample_adata.obs['leiden'] == cluster]
