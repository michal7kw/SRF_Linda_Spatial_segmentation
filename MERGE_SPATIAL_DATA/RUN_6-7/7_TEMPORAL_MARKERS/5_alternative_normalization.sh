#!/bin/bash
#SBATCH --job-name=5_alternative_normalization
#SBATCH --output=logs/5_alternative_normalization.out
#SBATCH --error=logs/5_alternative_normalization.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
mkdir -p MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/5_alternative_normalization
mkdir -p logs
...
python MERGE_SPATIAL_DATA/RUN_6-7/7_TEMPORAL_MARKERS/5_alternative_normalization.py
...
if [ $? -eq 0 ]; then
    ...
    echo "- Individual results: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/5_alternative_normalization/"
    echo "- Volcano plots: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/5_alternative_normalization/volcano_plot_*.png"
    echo "- Heatmaps: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/5_alternative_normalization/heatmap_*.png"
    echo "- Combined results: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/5_alternative_normalization/combined_de_results_all_timepoints.csv"
    echo "- Summary statistics: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/5_alternative_normalization/summary_statistics.csv"
    echo "- Quality control report: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/5_alternative_normalization/quality_control_report.txt"
    echo "- Batch correction plots: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/5_alternative_normalization/batch_correction_*.png"
    echo "================================================================"
else
    echo "================================================================"
