#!/bin/bash
#SBATCH --job-name=1_roi_based_temporal_markers_analysis
#SBATCH --output=logs/1_roi_based_temporal_markers_analysis.out
#SBATCH --error=logs/1_roi_based_temporal_markers_analysis.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
mkdir -p MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/1_roi_based_temporal_markers_analysis
mkdir -p logs
...
python MERGE_SPATIAL_DATA/RUN_6-7/7_TEMPORAL_MARKERS/1_roi_based_temporal_markers_analysis.py
...
if [ $? -eq 0 ]; then
    ...
    echo "- Individual bin expression matrices: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/1_roi_based_temporal_markers_analysis/roi_bin_expression_*.csv"
    echo "- Bin metadata: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/1_roi_based_temporal_markers_analysis/roi_bin_metadata_*.csv"
    echo "- DE results per time point: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/1_roi_based_temporal_markers_analysis/roi_de_results_*_mut_vs_ctrl.csv"
    echo "- Volcano plots: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/1_roi_based_temporal_markers_analysis/roi_volcano_plot_*.png"
    echo "- Spatial bin plots: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/1_roi_based_temporal_markers_analysis/roi_spatial_bins_*.png"
    echo "- Combined results: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/1_roi_based_temporal_markers_analysis/roi_combined_results_all_timepoints.csv"
    echo "- Summary statistics: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/1_roi_based_temporal_markers_analysis/roi_summary_statistics.csv"
    echo "- Analysis report: MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/1_roi_based_temporal_markers_analysis/roi_analysis_summary.txt"
    echo "================================================================"
    echo ""
    ...
    echo ""
    echo "Next steps:"
    echo "1. Compare results with cell-based analysis in MERGE_SPATIAL_DATA/7_TEMPORAL_MARKERS/results/"
